 ...
 * Looked up with Object.hasOwn, never plain truthiness: a bare `SYMBOLS[name]`
 * also finds inherited members, so `\toString` would emit a function.
 */
const SYMBOLS = {
  ...
  while (i < src.length) {
    ...
    if (ch === '\\') {
      ...
      if (cmd) {
        ...
        if (Object.hasOwn(SYMBOLS, name)) { atoms.push(SYMBOLS[name]); continue; }
      ...
      // Escaped punctuation — `\%`, `\$`, `\_`, `\\` — all emit the
      // character itself, so no per-character table is needed.
      const esc = rest[1];
      if (esc !== undefined) {
        atoms.push(escapeHtml(esc));
        i += 2;
        continue;
      }
      i += 1;
